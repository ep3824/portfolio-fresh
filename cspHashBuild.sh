...
if [ -f "$HASHES_FILE" ]; then
    ...
    if [ -n "$script_hashes" ]; then
        CSP_HEADER+=" script-src 'self' $script_hashes;"
    fi
    ...
    if [ -n "$style_hashes" ]; then
        CSP_HEADER+=" style-src 'self' $style_hashes;"
    fi
else
    # Default fallbacks if no hashes are generated
    CSP_HEADER+=" script-src 'self'; style-src 'self';"
fi

# Additional directives
CSP_HEADER+=" img-src 'self' https:; font-src 'self' https:; connect-src 'self' https:;"
